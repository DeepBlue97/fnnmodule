...
class MovenetLoss(torch.nn.Module):
    def __init__(self, use_target_weight=False, target_weight=[1], center_weight_path='', num_classes=-1):
        ...
        self.boneloss = JointBoneLoss(self.num_classes)
    ...
    def forward(self, output, target, kps_mask):
        ...
        heatmaps = target[:,:self.num_classes,:,:]
        centers = target[:,self.num_classes:self.num_classes+1,:,:]
        regs = target[:,self.num_classes:self.num_classes*3+1,:,:]
        offsets = target[:,self.num_classes*3+1:,:,:]
